...
static void header(const W98_FRAME*f,w98_u8 h[28]) {
    memcpy(h,"W98M",4);p16(h+4,1);p16(h+6,f->type);p32(h+8,f->flags);p32(h+12,f->stream_id);
...
int w98_send_frame(SOCKET s,unsigned short type,unsigned long flags,unsigned long stream,
 unsigned long lo,unsigned long hi,const void*payload,unsigned long len,const w98_u8*key,const char*dir) {
    W98_FRAME f;w98_u8 h[28],mac[32];f.type=type;f.flags=flags;f.stream_id=stream;f.seq_lo=lo;f.seq_hi=hi;f.payload_len=len;header(&f,h);
    if(key)w98_hmac_sha256(key,32,dir,strlen(dir),h,28,payload,len,mac);else memset(mac,0,32);
    return all_send(s,h,28)&&(!len||all_send(s,payload,len))&&all_send(s,mac,32);
}
int w98_recv_frame(SOCKET s,W98_FRAME*f,w98_u8**payload,w98_u8 mac[32]) {
    w98_u8 h[28];unsigned long max;if(!all_recv(s,h,28)||memcmp(h,"W98M",4)||g16(h+4)!=1)return 0;
    ...
    *payload=(w98_u8*)malloc(f->payload_len+1);if(!*payload)return 0;
    ...
    if(!all_recv(s,mac,32)){free(*payload);return 0;}return 1;
}
int w98_frame_ready(SOCKET s) {
    ...
    if(memcmp(h,"W98M",4)||g16(h+4)!=1)return 1;len=g32(h+24);max=g16(h+6)==W98_F_DATA?W98_MAX_DATA:W98_MAX_CONTROL;if(len>max)return 1;
    needed=28UL+len+32UL;return avail>=needed?1:0;
}
int w98_verify_frame(const W98_FRAME*f,const void*p,const w98_u8 mac[32],const w98_u8 key[32],const char*dir) {
    w98_u8 h[28],want[32],x=0;int i;header(f,h);w98_hmac_sha256(key,32,dir,strlen(dir),h,28,p,f->payload_len,want);
    for(i=0;i<32;i++)x|=(w98_u8)(want[i]^mac[i]);return x==0;
}
