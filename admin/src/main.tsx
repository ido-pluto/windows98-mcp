...
type Settings = { port: number; upstreamEnabled: boolean; upstreamHost: string; upstreamPort: number };
...
const defaultSettings: Settings = { port: 9898, upstreamEnabled: false, upstreamHost: "", upstreamPort: 9898 };
...
function App() {
  ...
  async function applySettings() {
    if (!Number.isInteger(settings.port) || settings.port < 1 || settings.port > 65535 || !Number.isInteger(settings.upstreamPort) || settings.upstreamPort < 1 || settings.upstreamPort > 65535) { setNotice("Ports must be between 1 and 65535."); return; }
    ...
    try { await invoke("save_settings", { settings }); await invoke("restart_broker", { settings }); setNotice(settings.upstreamEnabled ? `Connecting to ${settings.upstreamHost}:${settings.upstreamPort}…` : `Listening on port ${settings.port}.`); await refresh(); }
    catch (error) { setNotice(error instanceof Error ? error.message : String(error)); }
  }
